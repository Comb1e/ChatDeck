...
- **真实内容交接**：张嘴 200ms + 外壳增长 500ms，共用一个 0→1 进度；倒放只改变进度方向。增长时只有空外壳，完成后显示聊天和按钮。收起方向：鲸鱼窗口先显示在外壳位置、外壳以 0 透明度待命，play 后 140ms 淡入盖住真实悬浮窗（covered 回执），主进程收到 covered 才隐藏悬浮窗——UI 平滑溶解进外壳而非一帧硬切。展开方向：悬浮窗 show 会播 Windows DWM 显示过渡（~200ms 淡入），而它此刻在鲸鱼窗口之下，外壳必须等过渡走完（retireDelayMs 300ms）再清空并隐藏，否则半透明中间态直接暴露（整窗幽灵闪烁）。WebContentsView 不参与 CSS 变形，也不为切换而卸载或重新导航。
