...
- **鲸鱼窗口**：覆盖当前显示器工作区的透明窗口，兼作变形动画舞台。平时只有鲸鱼/气泡命中时接管鼠标；动画期间只有当前外形接管鼠标，点击可立即反向。收起到副屏后，鲸鱼继续在该屏活动。调试:`CHATDECK_WHALE_PINNED=1` 启动进入钉住模式(whale.html?pinned=1),鲸鱼停在初始位不自主游动,供形态/截图测试复现。
- **悬浮窗**：原生窗口 384×644 DIP，左/上各 24 DIP 给尾巴留空间；内面板 360×620。聊天视图为 `(34,108,340,484)`，Vue 布局和主进程共用尺寸。蓝色头部、边框、白眼睛和外伸尾巴来自共用 SVG 画笔。
- **真实内容交接**：张嘴 200ms + 外壳增长 500ms，共用一个 0→1 进度；倒放只改变进度方向。增长时只有空外壳，完成后显示聊天和按钮。收起方向：鲸鱼窗口先显示在外壳位置、外壳以 0 透明度待命，play 后 140ms 淡入盖住真实悬浮窗，covered 回执延迟到 340ms（给鲸鱼窗口自身 re-show 的 DWM 显示过渡留余量——过早隐藏悬浮窗会在半透明窗口下面把 UI 硬切掉），主进程收到 covered 才隐藏悬浮窗并开始倒放；收起中途反向时外壳改为 120ms 淡出，悬浮窗保持可见。展开方向：悬浮窗 show 会播 Windows DWM 显示过渡（~200ms 淡入），而它此刻在鲸鱼窗口之下，外壳必须等过渡走完（retireDelayMs 300ms）再清空并隐藏，否则半透明中间态直接暴露（整窗幽灵闪烁）。鲸鱼窗口 re-show 一律 showInactive（全屏透明覆盖层不抢输入焦点）。WebContentsView 不参与 CSS 变形，也不为切换而卸载或重新导航。
